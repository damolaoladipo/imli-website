...
export async function generateMetadata({
  ...
  if (!page || slug.join("/") !== "digital-storytelling-for-peace-building") {
    return {
      title: "Programme Not Found",
      description: "The requested AFAX-P programme could not be found.",
    };
  }

  const data = page.data as ProjectData;
  const canonical = `${siteConfig.url}/afax-p/${slug.join("/")}`;
  const ogImage = absoluteOgImageUrl(data.heroImage);
  const title = data.acronym ? `${data.acronym} — ${data.title}` : data.title;
